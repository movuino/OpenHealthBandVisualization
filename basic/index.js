    ...
    for (const service of services) {
      ...
      characteristics.forEach((characteristic) => {
        ...
        if (characteristic.uuid == "00001201-0000-1000-8000-00805f9b34fb") {
          ...
          errorCharacteristic.addEventListener(
            ...
            (event) => {
              ...
              document.getElementById("IMU-state").classList = IMUError
                ? "status-ko"
                : "status-ok"; // Update DOM
              document.getElementById("PPG-state").classList = PPG86Error
                ? "status-ko"
                : "status-ok";
            }
          );
        ...
        if (characteristic.uuid == "00001102-0000-1000-8000-00805f9b34fb") {
          ...
          ACCCharacteristic.addEventListener(
            "characteristicvaluechanged",
            (event) => {
              ...
              var v = (value.getUint8(5) << 8) | value.getUint8(6);
              if (v & 0x8000) {
                v = v - 0x10000; // Convert to a signed 16-bit value
              }
              var x = (v * -1 * 16) / 0x8000;

              v = (value.getUint8(7) << 8) | value.getUint8(8);
              if (v & 0x8000) {
                v = v - 0x10000; // Convert to a signed 16-bit value
              }
              var y = (v * -1 * 16) / 0x8000;

              v = (value.getUint8(9) << 8) | value.getUint8(10);
              if (v & 0x8000) {
                v = v - 0x10000; // Convert to a signed 16-bit value
              }
              var z = (v * -1 * 16) / 0x8000;

              // console.log(value.getUint8(9), value.getUint8(10), z);
              console.log(x, y, z);
        ...
        if (characteristic.uuid == "00001103-0000-1000-8000-00805f9b34fb") {
          ...
          GYRCharacteristic.addEventListener(
            "characteristicvaluechanged",
            (event) => {
              const value = event.target.value;
              const timestamp =
              ...
              var v1 = (value.getUint8(5) << 8) | value.getUint8(6);
              if (v & 0x8000) {
                v = v - 0x10000; // Convert to a signed 16-bit value
              }
              var x = (v1 * -1 * 2000) / 0x8000;

              v1 = (value.getUint8(7) << 8) | value.getUint8(8);
              if (v & 0x8000) {
                v = v - 0x10000; // Convert to a signed 16-bit value
              }
              var y = (v1 * -1 * 2000) / 0x8000;

              v1 = (value.getUint8(9) << 8) | value.getUint8(10);
              if (v & 0x8000) {
                v = v - 0x10000; // Convert to a signed 16-bit value
              }
        ...
        if (characteristic.uuid == "00001104-0000-1000-8000-00805f9b34fb") {
          ...
          MAGCharacteristic.addEventListener(
            "characteristicvaluechanged",
            (event) => {
              const value = event.target.value;
              ...
              var x =
                (value.getInt8(4) << 8) |
                (value.getInt8(5) * (((174 - 128) * 0.5) / 128 + 1));
              var y =
                (value.getInt8(6) << 8) |
                (value.getInt8(7) * (((177 - 128) * 0.5) / 128 + 1));

              var z =
                (value.getInt8(8) << 8) |
                (value.getInt8(9) * (((164 - 128) * 0.5) / 128 + 1));
              //console.log(x,y,z)
        ...
        if (characteristic.uuid == "00001305-0000-1000-8000-00805f9b34fb") {
          ...
          LEDSEQ1A_PD1CHARACTERISTIC2.addEventListener(
            "characteristicvaluechanged",
            LEDSEQ1A_PD1CHARACTERISTIC2Changed
          );
        ...
        if (characteristic.uuid == "00001307-0000-1000-8000-00805f9b34fb") {
          ...
          LEDSEQ1A_PD2CHARACTERISTIC2.addEventListener(
            "characteristicvaluechanged",
            LEDSEQ1A_PD2CHARACTERISTIC2Changed
          );
        ...
        if (characteristic.uuid == "00001313-0000-1000-8000-00805f9b34fb") {
          ...
          SNR1_2PD1CHARACTERISTIC2.addEventListener(
            "characteristicvaluechanged",
            SNR1_2PD1CHARACTERISTIC2Changed
          );
        ...
        if (characteristic.uuid == "00001314-0000-1000-8000-00805f9b34fb") {
          ...
          SNR1_2PD2CHARACTERISTIC2.addEventListener(
            "characteristicvaluechanged",
            SNR1_2PD2CHARACTERISTIC2Changed
          );
...
function sendValues() {
  ...
  var valeurselectionnee1 =
    selectElmt1.options[selectElmt1.selectedIndex].value;
  console.log("valeurselectionnee1 " + valeurselectionnee1);
  if (valeurselectionnee1 == 25) {
    inputSampleRate[0] = 0;
  } else if (valeurselectionnee1 == 50) {
    inputSampleRate[0] = 1;
  } else if (valeurselectionnee1 == 84) {
    inputSampleRate[0] = 2;
  } else if (valeurselectionnee1 == 100) {
    inputSampleRate[0] = 3;
  } else if (valeurselectionnee1 == 200) {
    inputSampleRate[0] = 4;
  } else if (valeurselectionnee1 == 400) {
    inputSampleRate[0] = 5;
  } else if (valeurselectionnee1 == 128) {
    inputSampleRate[0] = 14;
  } else if (valeurselectionnee1 == 256) {
    inputSampleRate[0] = 15;
  } else if (valeurselectionnee1 == 512) {
    inputSampleRate[0] = 16;
  } else if (valeurselectionnee1 == 1024) {
    inputSampleRate[0] = 17;
  } else if (valeurselectionnee1 == 2048) {
    inputSampleRate[0] = 18;
  } else {
    inputSampleRate[0] = 19;
  }
  ...
  if (valeurselectionnee == 1) {
    ...
    inputSampleAvg[0] = 1;
  } else if (valeurselectionnee == 4) {
    inputSampleAvg[0] = 2;
  } else if (valeurselectionnee == 8) {
    inputSampleAvg[0] = 3;
  } else if (valeurselectionnee == 16) {
    inputSampleAvg[0] = 4;
  } else if (valeurselectionnee == 32) {
    inputSampleAvg[0] = 5;
  } else if (valeurselectionnee == 64) {
    inputSampleAvg[0] = 6;
  } else {
    inputSampleAvg[0] = 7;
  }
...
window.onload = () => {
  ...
  console.log(
    navigator.bluetooth ? "Web-BLE Available" : "Web-BLE Unavailable"
  );
  document
  ...
  IMUAccSmoothie.addTimeSeries(IMUAccXLine, {
    strokeStyle: "rgb(255, 0, 0)",
    lineWidth: 3,
  });
  IMUAccSmoothie.addTimeSeries(IMUAccYLine, {
    strokeStyle: "rgb(0, 255, 0)",
    lineWidth: 3,
  });
  IMUAccSmoothie.addTimeSeries(IMUAccZLine, {
    strokeStyle: "rgb(0, 0, 255)",
    lineWidth: 3,
  });
  IMUAccSmoothie.streamTo(document.getElementById("smoothie-imu-acc"));

  IMUGyrSmoothie.addTimeSeries(IMUGyrXLine, {
    strokeStyle: "rgb(255, 0, 0)",
    lineWidth: 3,
  });
  IMUGyrSmoothie.addTimeSeries(IMUGyrYLine, {
    strokeStyle: "rgb(0, 255, 0)",
    lineWidth: 3,
  });
  IMUGyrSmoothie.addTimeSeries(IMUGyrZLine, {
    strokeStyle: "rgb(0, 0, 255)",
    lineWidth: 3,
  });
  IMUGyrSmoothie.streamTo(document.getElementById("smoothie-imu-gyr"));

  IMUMagSmoothie.addTimeSeries(IMUMagXLine, {
    strokeStyle: "rgb(255, 0, 0)",
    lineWidth: 3,
  });
  IMUMagSmoothie.addTimeSeries(IMUMagYLine, {
    strokeStyle: "rgb(0, 255, 0)",
    lineWidth: 3,
  });
  IMUMagSmoothie.addTimeSeries(IMUMagZLine, {
    strokeStyle: "rgb(0, 0, 255)",
    lineWidth: 3,
  });
  ...
  LEDSEQ1A_PD1Characteristic2Smoothie.addTimeSeries(
    LEDSEQ1A_PD1Characteristic2Line,
    { strokeStyle: "rgb(255, 0, 0)", lineWidth: 3 }
  ); // Link TimeSeries to the SmoothieChart object and add a bit of color
  LEDSEQ1A_PD1Characteristic2Smoothie.streamTo(
    document.getElementById("smoothie-pd1-led")
  ); // Link the SoothieChart object to its canvas in the HTML
  ...
  LEDSEQ1A_PD2Characteristic2Smoothie.addTimeSeries(
    LEDSEQ1A_PD2Characteristic2Line,
    { strokeStyle: "rgb(255, 0, 0)", lineWidth: 3 }
  ); // Link TimeSeries to the SmoothieChart object and add a bit of color
  LEDSEQ1A_PD2Characteristic2Smoothie.streamTo(
    document.getElementById("smoothie-pd2-led")
  ); // Link the SoothieChart object to its canvas in the HTML
};
...
async function LEDSEQ1A_PD1CHARACTERISTIC2Changed(event) {
  ...
  const timestamp =
    ...
    (value.getUint8(1) << 16) |
    (value.getUint8(2) << 8) |
    value.getUint8(3); // shifts 4 Uint8 to make a Uint32
  const data1 =
    (value.getUint8(4) << 24) |
    (value.getUint8(5) << 16) |
    (value.getUint8(6) << 8) |
    value.getUint8(7); // shifts 4 Uint8 to make a Uint32
  const data2 =
    (value.getUint8(8) << 24) |
    (value.getUint8(9) << 16) |
    (value.getUint8(10) << 8) |
    value.getUint8(11); // shifts 4 Uint8 to make a Uint32
...
async function SNR1_2PD2CHARACTERISTIC2Changed(event) {
  const value = event.target.value;
  const snr1_2PD2CHARACTERISTIC2 =
    (value.getUint8(0) << 24) |
    (value.getUint8(1) << 16) |
    (value.getUint8(2) << 8) |
    value.getUint8(3); // shifts 4 Uint8 to make a Uint32
